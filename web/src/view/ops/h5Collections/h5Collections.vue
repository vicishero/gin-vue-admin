  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="searchForm" :inline="true" :model="searchInfo">
        ...
        <el-form-item label="贴文ID">
          <el-input v-model="searchInfo.postId" placeholder="贴文ID" style="width:140px" />
        </el-form-item>
        <el-form-item>
    ...
    <div class="gva-table-box">
      <el-table :data="tableData" row-key="ID">
        ...
        <el-table-column align="left" label="用户ID" min-width="100" prop="userId" />
        <el-table-column align="left" label="贴文ID" min-width="100" prop="postId" />
        <el-table-column align="left" label="收藏时间" min-width="170">
...
<script setup>
import { getH5CollectionList, deleteH5Collection } from '@/api/h5Admin'
import { ref, reactive } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { formatDate } from '@/utils/format'

defineOptions({ name: 'h5Collections' })

const page = ref(1), total = ref(0), pageSize = ref(10), tableData = ref([])
const searchInfo = reactive({ userId: '', postId: '' })
...
const getTableData = async () => {
  ...
  const res = await getH5CollectionList(params)
  if (res.code === 0) { tableData.value = res.data.list; total.value = res.data.total }
}
getTableData()

const onSearch = () => { page.value = 1; getTableData() }
const onReset = () => { searchInfo.userId = ''; searchInfo.postId = ''; page.value = 1; getTableData() }
const handleSizeChange = (v) => { pageSize.value = v; getTableData() }
const handleCurrentChange = (v) => { page.value = v; getTableData() }

const deleteFunc = async (row) => {
  ElMessageBox.confirm('确定要删除该收藏记录吗?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(async () => {
    const res = await deleteH5Collection({ ID: row.ID })
